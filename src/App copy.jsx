...
function App() {
  ...
  const toggleDisplayMode = () => {
    if (inlineMode) {   // Wenn der Inline-Modus eingeschaltet ist also Inline -> 2-zeilig
...
//prüfen, ob eine Akkordzeile vorliegt über das Akkordzeichen "("
        if(!line.includes("(")) {
          converted.push(line);    // Diese Zeile als Textzeile zuweisen, ohne Akkorde
          continue; // Durchlauf abbrechen und neu starten
        }
        ...
        while (i < line.length) {
          ...
          if (inChord) {          // Wenn die Akkord-Behandlung läuft
            ...
            if(akkordLaenge > 1) {  // Wenn die Akkordbezeichnung mehr als ein Zeichen ist ....
               akkordLaenge--       // um 1 reduzieren, bis eine Akkordlänge von 1 erreicht ist. 
            } else {     
              chordLine += " "      // erst dann mit Leerzeichen auffüllen, das sich ansonsten die Akkorde gegenüber der Testzeile nach hinten verschieben.
  ...
  function isLikelyChordLine(line) {
    ...
    const chordRegex = /^[A-G](#|b)?(5|m|maj7|m7|7|sus2|sus4|dim|aug|add\d*)?(\/[A-G](#|b)?)?$/;
...
//Tokenizer (ohne Fokus auf Zeitstempel)
function tokenizeChordLine(line) {
  ...
  for (const part of parts) {
    if (part === "NC") {
      tokens.push({ type: "no-chord" });
    } else if (isChord(part)) {
...
function pairLines(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isChordLine(chordLine) && textLine) {
      result.push({
        type: "pair",
        chords: tokenizeChordLine(chordLine),
        text: textLine
      });
      i++;
    } else {
...
function renderInline(pair) {
  const chordLine = pair.rawChordLine;
  const textLine = pair.text;
  ...
  while (t < maxLength) {
    if (paddedChord[t] !== " ") {
      ...
      while (k < maxLength && paddedChord[k] !== " ") {
        chord += paddedChord[k];
        k++;
      }

      // 🔥 HIER IST DER FIX FÜR NC
      if (chord === "NC") {
        // Option A: komplett ignorieren
        // result += "";

        // Option B: sichtbar lassen
        result += "(NC)";
      } else {
        result += `(${chord})`;
      }
